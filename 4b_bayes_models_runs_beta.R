...
cat("model{
   ...
   for(z in 1:12){
    ...
    # predict on actual X data
    for(z in 1:P){
    pred_X_1[z] <- B0_habitat_p[1] + inprod(beta_point_p,X_point[z,])
    pred_X_2[z] <- B0_habitat_p[2] + inprod(beta_point_p,X_point[z,])
    
    }
    

    }  ", file="beta_hierarchical.jags")

# Pocillopora & TotN ------------------------------------------------------
temp <- moorea[moorea$Taxa=="Pocillopora",]
temp <- temp[!is.na(temp$Percent_bleached),]
temp <- temp[!is.na(temp$avgTotN),]
# temp <- temp[sample(nrow(temp),300),]

# subset for only severity (remove zeros)
...
out <- clusterEvalQ(cl,{
  library(rjags)
  tmod <- jags.model("beta_hierarchical.jags", data= jd, n.chains=1, n.adapt=n.adapt,inits = initFunc())
  ...
  zmCore <- coda.samples(tmod, c(
    ...
    'pval.cv','pval.fit','pval.min','pval.max',
    'pred_X_1','pred_X_2'),n.iter=n.iter, n.thin=1)
